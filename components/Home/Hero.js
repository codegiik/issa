...
export function Tile({ article, active, setActive, unsetActive, index }) {
  ...
  const color = index % 2 == 0 ? colors['cedar']['500'] : colors['cedar']['600']
  ...
  return (
    <div className={[style.carouselTile, active ? style.tileActive : null].join(' ')} style={{
    ...
    if(!this.timer)
      this.timer = setInterval(() => {
        if(!this.state.disableLoop){
          this.setState({
            ...this.state,
            activeTile: (this.state.activeTile + 1) % (MAX_NEWS_TO_SHOW || this.props.news.length)
          })
        }
      }, 10000)
  }
